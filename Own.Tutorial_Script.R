...
#mutate(AlaskaCoords$lat ~ 60.63 & long ~ -147.3, subregion = "Prince William Sound")
...
#The line just below is used for creating the map of Alaska?
Alaska_Map <- left_join(AlaskaCoords, KillerWhaleData, by = "subregion")
                        
SEAandPWS_Coords <- cbind(AlaskaCoordsPWS, AlaskaCoordsSEA)


mutate

left_join(AlaskaCoords)




       
(AlaskaOrcas_Map1 <- ggplot(AlaskaCoords, aes(x = long, y = lat, group = group)) + 
